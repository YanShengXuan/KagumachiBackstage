    ...
    const client = new Client({
      ...
      onConnect: (frame) => {
        ...
        client.subscribe("/topic/unreadBackMessages", (messageOutput) => {
          ...
          // console.log(
          //   "Received unread back messages from server:",
          //   unreadMessages
          // );
          const unreadUserIds = unreadMessages.map((message) =>
            ...
            return (
              ...
              >
                ...
                {newMessageUsers.includes(user.id) && (
                  <span className="absolute top-[0.5rem] right-[1rem] text-xl text-red-500 font-bold">
                    !
                  </span>
                )}
              </li>
        ...
        <div className="flex justify-center">
          <button
            className={`px-20 py-3 border border-gray-300 rounded-md bg-[#5783db] text-gray-100 hover:bg-[#55c2da] hover:text-gray-100 hover:cursor-pointer ${
              selectedUser ? "" : "opacity-50 hover:cursor-not-allowed"
            }`}
            onClick={sendMessage}
            disabled={!selectedUser}
          >
